...
@Delete("DELETE FROM qrtz_paused_trigger_grps WHERE SCHED_NAME=#{arg0}")
...
fun UpdateByPrimaryKey(model:qrtz_paused_trigger_grps_dto)
        

@Update("""<script>
            UPDATE qrtz_paused_trigger_grps SET
            <if test="SCHED_NAME != null">  SCHED_NAME=#{SCHED_NAME,jdbcType=String},</if>
        <if test="TRIGGER_GROUP != null">  TRIGGER_GROUP=#{TRIGGER_GROUP,jdbcType=String}
              WHERE SCHED_NAME=#{SCHED_NAME,jdbcType=String}
            </script>""")
fun UpdateByPrimaryKeySelective(model:qrtz_paused_trigger_grps_dto)
