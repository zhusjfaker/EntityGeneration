...
@Delete("DELETE FROM qrtz_scheduler_state WHERE SCHED_NAME=#{arg0}")
...
@Update("""<script>
        ...
        LAST_CHECKIN_TIME=#{LAST_CHECKIN_TIME,jdbcType=Long},
        CHECKIN_INTERVAL=#{CHECKIN_INTERVAL,jdbcType=Long}
        WHERE SCHED_NAME=#{SCHED_NAME,jdbcType=String}
        </script>""")
fun UpdateByPrimaryKey(model:qrtz_scheduler_state_dto)
...
@Update("""<script>
        ...
        <if test="LAST_CHECKIN_TIME != null">  LAST_CHECKIN_TIME=#{LAST_CHECKIN_TIME,jdbcType=Long},</if>
        <if test="CHECKIN_INTERVAL != null">  CHECKIN_INTERVAL=#{CHECKIN_INTERVAL,jdbcType=Long}
              WHERE SCHED_NAME=#{SCHED_NAME,jdbcType=String}
            </script>""")
fun UpdateByPrimaryKeySelective(model:qrtz_scheduler_state_dto)
