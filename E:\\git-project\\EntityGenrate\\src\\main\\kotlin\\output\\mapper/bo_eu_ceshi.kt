...
fun ConditionalQuery(model:bo_eu_ceshi_dto):ArrayList<bo_eu_ceshi_dto>
...
fun ConditionalQueryByKey(model:bo_eu_ceshi_dto):bo_eu_ceshi_dto?
                


@Insert("""<script>
    insert into bo_eu_ceshi
    (ID,ORGID,BINDID,CREATEDATE,CREATEUSER,UPDATEDATE,UPDATEUSER,PROCESSDEFID,ISEND)
    values
    (#{ID},#{ORGID},#{BINDID},#{CREATEDATE},#{CREATEUSER},#{UPDATEDATE},#{UPDATEUSER},#{PROCESSDEFID},#{ISEND})
</script>""")
fun Insert(model:bo_eu_ceshi_dto):Unit
...
fun InsertSelective(model:bo_eu_ceshi_dto):Unit
                


@Delete("DELETE FROM bo_eu_ceshi WHERE ID=#{arg0}")
fun DeleteByPrimaryKey(ID:String)
        

@Update("""<script>
        UPDATE bo_eu_ceshi SET
...
fun UpdateByPrimaryKey(model:bo_eu_ceshi_dto)
...
@Update("""<script>
        ...
        <if test="ISEND != null">  ISEND=#{ISEND,jdbcType=Integer}
              WHERE ID=#{ID,jdbcType=String}
            </script>""")
fun UpdateByPrimaryKeySelective(model:bo_eu_ceshi_dto)
